...
impl<const MAIN_CHANNEL_SELECTION: u8> ReadCd<Lba, Mode2Form1<MAIN_CHANNEL_SELECTION>>
where
...
impl<O: OpCodeDef, A: ReadCdAddress, const MAIN_CHANNEL_SELECTION: u8> ReadCommand<O>
    for ReadCd<A, Mode2Form1<MAIN_CHANNEL_SELECTION>>
where
    Mode2Form1<MAIN_CHANNEL_SELECTION>: SectorSelection,
    ReadCd<A, Mode2Form1<MAIN_CHANNEL_SELECTION>>: Command<O>,
{
    type Len = u64;
    type Response<'a> = ();
    type Error = ();
...
pub struct Mode2Form1<const MAIN_CHANNEL_SELECTION: u8>;
impl_sector_selection!(
    Mode2Form1,
    private::SectorType::Mode2Form1,
    [
